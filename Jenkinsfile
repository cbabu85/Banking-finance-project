pipeline {
  ...
  stages {
    ...
    stage('DockerLogin') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'Docker-login-ID', passwordVariable: 'docker-password', usernameVariable: 'docker-login-hub')]) {
        sh "docker login -u ${docker-login-hub} -p ${docker_password}"
            }
        }
    }
